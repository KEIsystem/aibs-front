...
function PrimaryTumorInfoPanel({
  ...
  pgrPS, setPgrPS,
  pgrIS, setPgrIS,
  oncotypeDX, setOncotypeDX,
  oncotypeNotTested, setOncotypeNotTested
}) {
  ...
  return (
    ...
    <div className="mt-2">
      ...
      {typeof oncotypeDX !== 'undefined' && typeof setOncotypeDX === 'function' && (
        <div className="mb-4 flex items-center gap-4">
          <label className="text-sm font-bold mr-2">Oncotype DXスコア：</label>
          <input
            type="number"
            min="0"
            max="100"
            value={oncotypeDX}
            onChange={(e) => setOncotypeDX(e.target.value)}
            disabled={oncotypeNotTested}
            className="border rounded px-2 py-1 w-20 h-8"
          />
          <label className="flex items-center ml-2">
            <input
              type="checkbox"
              checked={oncotypeNotTested}
              onChange={(e) => {
                setOncotypeNotTested(e.target.checked);
                if (e.target.checked) setOncotypeDX('');
              }}
              className="mr-1"
            />
            未検査
