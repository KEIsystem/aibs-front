...
const drugCategories = {
  ...
  targeted: ['Olaparib', 'Talazoparib', 'Atezolizumab', 'Pembrolizumab', 'Bev','SG', 'Dato-Dxd', 'Dmab'],
...
function PostProgressionTreatmentForm() {
  ...
  const [oncotypeDX, setOncotypeDX] = useState('');
  const [oncotypeNotTested, setOncotypeNotTested] = useState(false);
  ...
  const handleOutcomeChange = (index, value) => {
    ...
    if (value === "PD") {
      if (index === 0) {
        // 1次治療のとき：再発時情報からコピー
        updated[index].metastasisSites = { ...metastasisSites };
      } else {
        // 2次治療以降：直前のPD時転移部位を探してコピー
        const priorPD = treatments
          .slice(0, index)
          .reverse()
          .find(t => t.outcome === "PD" && t.metastasisSites);
        updated[index].metastasisSites = priorPD ? { ...priorPD.metastasisSites } : {
          local: false, local_ln: false, distant_ln: false,
          lung: false, liver: false, bone: false,
          brain: false, other: false
        };
      }

      // other部位が false のときは自由記述をクリア
      if (!updated[index].metastasisSites.other) {
  ...
  const handleSubmit = async () => {
    ...
    const formData = {
      ...
      primary_tumor_info: {
        ...
        grade,
        oncotypeDX: oncotypeNotTested ? null : Number(oncotypeDX)
      },
      ...
      {!isDeNovo && (
        <>
          <PrimaryTumorInfoPanel
            ...
            oncotypeDX={oncotypeDX} setOncotypeDX={setOncotypeDX}
            oncotypeNotTested={oncotypeNotTested} setOncotypeNotTested={setOncotypeNotTested}
          />
